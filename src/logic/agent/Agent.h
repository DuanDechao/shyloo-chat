...
class LogicSessionServer : public sl::api::ITcpServer{
public:
	LogicSessionServer(Agent* pAgent) :_agent(pAgent){}
	virtual ~LogicSessionServer(){}
	virtual sl::api::ITcpSession* mallocTcpSession(sl::api::IKernel* pKernel);

	inline void recover(LogicSession* session){
		s_pool.recover(session);
	}

private:
	Agent* _agent;
	static sl::SLPool<LogicSession> s_pool;
};
...
public:
	...
	int64 onOpen(SessionBase* pSession);
...
private:
	...
	LogicSessionServer* _logicServer;
	std::unordered_map<int64, SessionBase*> _sessions;
};
#endif