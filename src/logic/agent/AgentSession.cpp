...
int32 LogicSession::onRecv(sl::api::IKernel* pKernel, const char* pContext, int dwLen){
	return _agent->onRecv(_id, pContext, dwLen);
}

void LogicSession::onConnected(sl::api::IKernel* pKernel){
	_id = _agent->onOpen(this);
}

void LogicSession::onDisconnect(sl::api::IKernel* pKernel){
	_agent->onClose(_id);
	_agent->recover(this);
}